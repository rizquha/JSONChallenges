...
namespace TaskNumber2
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach(var item in user)
            {
                if((item.Customer.Name).Contains("Ari"))
                {
                    Console.WriteLine("Id Items : "+item.OrderId);
                }
            }
            ...
            int AriTotal()
            {
                ...
                foreach(var item in user)
                {
                    if((item.Customer.Name).Contains("Ari"))
                    {
                        ...
                        {
                            GrandTotal+=((items.Price)*(items.Qty));
                        }
                    }
                }
                return GrandTotal;
            ...
            int RirinTotal()
            {
                ...
                foreach(var item in user)
                {
                    if((item.Customer.Name).Contains("Ririn"))
                    {
                        foreach(var items in item.Items)
                        {
                            GrandTotal+=((items.Price)*(items.Qty));
            ...
            int AnnisTotal()
            {
                ...
                foreach(var item in user)
                {
                    if((item.Customer.Name).Contains("Annis"))
                    {
                        foreach(var items in item.Items)
                        {
                            GrandTotal+=((items.Price)*(items.Qty));
